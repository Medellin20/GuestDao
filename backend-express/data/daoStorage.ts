...
class DaoStorage {
  private storage: Dao[] = SHOULD_SEED ? [...serverMockDaos] : [];
  private idIndex: Map<string, number> = new Map();
  private autoriteIndex: Map<string, Dao[]> = new Map();

  constructor() {
    console.log("🏗️ Initializing DAO storage...");

    // Initialize with optional seeding
    this.storage = SHOULD_SEED ? [...serverMockDaos] : [];
    this.idIndex.clear();
    ...
    console.log("✅ DAO storage initialization complete");
  }
