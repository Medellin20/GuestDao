  ...
  > {
    ...
    for (const token of sessions) {
      try {
        ...
        result.push({ token, user: decoded });
      } catch (_) {
        result.push({ token, user: null });
      }
    }
    return result;
  }
