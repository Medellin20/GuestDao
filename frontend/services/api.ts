...
class ApiService {
  ...
  async deleteLastDao(): Promise<{ deletedId: string; numeroListe: string }> {
    try {
      // Primary admin endpoint mounted under /api/admin
      return await this.request<{ deletedId: string; numeroListe: string }>(
        "/admin/delete-last-dao",
        { method: "DELETE" },
      );
    } catch (err) {
      // Secondary: dao router admin endpoint (if available)
      try {
        return await this.request<{ deletedId: string; numeroListe: string }>(
          "/dao/admin/delete-last",
          { method: "DELETE" },
        );
      } catch (_) {
        // Fallback: determine last DAO and attempt standard delete (may be disabled)
        const last = await this.getLastDao();
        await this.request<void>(`/dao/${last.id}`, { method: "DELETE" });
        cacheService.delete("all-daos");
        cacheService.delete(`dao-${last.id}`);
        return { deletedId: last.id, numeroListe: last.numeroListe };
      }
    }
  }
